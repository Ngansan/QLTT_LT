      ...
      [
          {
            ...
            "DOANHSO": 13060000, 
            "bought_SP": ["But chi", "But bi", "So tay 500 trang"]
          },
          {
            ...
            "DOANHSO": 280000,
            "bought_SP": ["But chi", "Tap 200 giay mong", "Bong bang"]
          },
          {
...
db.getCollection('KHACHHANG').find({
  ...
  // lấy ra thông tin khách hàng: hoten, diachi, sdt, ngaysinh. Chỉ lấy ra 4 người, 
  // group theo tên
  db.getCollection('KHACHHANG').aggregate([
    {$project:{
        HOTEN: 1,
        DCHI: 1, 
        SODT: 1, 
  ...
  // kết 2 collection CTHD và SANPHAM
  db.getCollection('CTHD').aggregate( [
    {
       $lookup:
          {
            from: "SANPHAM",
